...
async function sendModerationLog(interaction, targetUser, reason, action) {
    ...
    // Replace ${action} before calling replacePlaceholders
    const rawTitle = messages.moderationLogs.title.replace('${action}', action);
    const rawMessage = messages.moderationLogs.message.replace('${action}', action);
